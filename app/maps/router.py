...
from app.maps.dependencies import MapFilterParams, get_valid_map, get_valid_map_id, get_valid_map_with_creator
...
async def download_map(map_obj: Map = Depends(get_valid_map)):
    ...
    if not os.path.exists(map_obj.map_url):
        raise HTTPException(status_code=404, detail="실제 파일이 서버에 존재하지 않습니다.")
